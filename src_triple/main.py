...
def pre_train(model, X1, A1, X2, A2, X3, A3):
    print("Pretraining fusion model...")
    optimizer = Adam(model.parameters(), lr=opt.args.lr)
    for epoch in tqdm.tqdm(range(opt.args.fus_epoch)):

        # input & output
        X_hat1, Z_hat1, A_hat1, X_hat2, Z_hat2, A_hat2, X_hat3, Z_hat3, A_hat3, _, _, _, _, _, _, cons = model(X1, A1, X2, A2, X3, A3,pretrain=True)

        L_DRR = drr_loss(cons)
        L_REC1 = reconstruction_loss(X1, A1, X_hat1, Z_hat1, A_hat1)
        L_REC2 = reconstruction_loss(X2, A2, X_hat2, Z_hat2, A_hat2)
        L_REC3 = reconstruction_loss(X3, A3, X_hat3, Z_hat3, A_hat3)

        loss = L_REC1 + L_REC2 + L_REC3 + L_DRR
...
def train(model, X1, A1, X2, A2, X3, A3, y):
    if not opt.args.pretrain:
        ...
        with torch.no_grad():
            _, _, _, _, _, _, _, _, _, _, _, _, Z1, Z2, Z3, _ = model(X1, A1, X2, A2, X3, A3)
        
        _, _, _, _, centers1 = clustering(Z1, y)
        _, _, _, _, centers2 = clustering(Z2, y)
        _, _, _, _, centers3 = clustering(Z3, y)
        ...
        model.cluster_centers3.data = torch.tensor(centers3).to(opt.args.device)
    ...
    for epoch in pbar:
        ...
        # input & output
        X_hat1, Z_hat1, A_hat1, X_hat2, Z_hat2, A_hat2, X_hat3, Z_hat3, A_hat3, Q1, Q2, Q3, Z1, Z2, Z3, cons = model(X1, A1, X2, A2, X3, A3)
        ...
        if opt.args.first_view == 'RNA':
            if epoch % 600 < 200:
                ...
                L_KL3 = distribution_loss(Q3, target_distribution(Q1[0].data))
            elif epoch % 600 < 400:
                L_KL1 = distribution_loss(Q1, target_distribution(Q2[0].data))
                L_KL2 = distribution_loss(Q2, target_distribution(Q2[0].data))
                L_KL3 = distribution_loss(Q3, target_distribution(Q2[0].data))
            else:
                L_KL1 = distribution_loss(Q1, target_distribution(Q3[0].data))
                L_KL2 = distribution_loss(Q2, target_distribution(Q3[0].data))
                L_KL3 = distribution_loss(Q3, target_distribution(Q3[0].data))
        else:
            if epoch % 600 < 200:
                L_KL1 = distribution_loss(Q1, target_distribution(Q2[0].data))
                L_KL2 = distribution_loss(Q2, target_distribution(Q2[0].data))
        ...
        loss = L_REC1 + L_REC2 + L_REC3 + L_DRR + opt.args.lambda3 * (L_KL1 + L_KL2 + L_KL3)
        ...
        ari, nmi, ami, acc, _ = assignment((Q1[0] + Q2[0] + Q3[0]).data, y)
...
if __name__ == '__main__':
    ...
    Xm, y, Am = load_data(opt.args.name, 'ADT', opt.args.method, opt.args.k, show_details=False)
    ...
    Xm = numpy_to_torch(Xm).to(opt.args.device)
    Am = numpy_to_torch(Am, sparse=True).to(opt.args.device)
    ...
    ae3 = AE(
        ae_n_enc_1=opt.args.ae_n_enc_1, ae_n_enc_2=opt.args.ae_n_enc_2,
        ae_n_dec_1=opt.args.ae_n_dec_1, ae_n_dec_2=opt.args.ae_n_dec_2,
        n_input=opt.args.n_d3, n_z=opt.args.n_z).to(opt.args.device)
    ...
    gae3 = IGAE(
        gae_n_enc_1=opt.args.gae_n_enc_1, gae_n_enc_2=opt.args.gae_n_enc_2,
        gae_n_dec_1=opt.args.gae_n_dec_1, gae_n_dec_2=opt.args.gae_n_dec_2,
        n_input=opt.args.n_d3, n_z=opt.args.n_z, dropout=opt.args.dropout).to(opt.args.device)
    ...
    if opt.args.pretrain:
        ...
        pretrain_gae(gae3, Xm, Am)

        model = scMIC(ae1, ae2, ae3, gae1, gae2, gae3, n_node=Xr.shape[0]).to(opt.args.device)

        pre_train(model, Xr, Ar, Xa, Aa, Xm, Am)

        train(model, Xr, Ar, Xa, Aa, Xm, Am, y)
    else:
        model = scMIC(ae1, ae2, ae3, gae1, gae2, gae3, n_node=Xr.shape[0]).to(opt.args.device)
